...
import { Plus, Trash2, Save, User, MapPin, FileText, Euro } from 'lucide-react';
...
export default function DevisForm({ initialData, onSubmit, onCancel, isLoading }: DevisFormProps) {
  const [formData, setFormData] = useState<DevisFormData>({
    ...
    dateValidite: initialData?.dateValidite || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
    lignes: initialData?.lignes || [defaultLigne()],
    notes: initialData?.notes || '',
    conditionsPaiement: initialData?.conditionsPaiement || 'Paiement à réception',
  });
  ...
  const handleLigneChange = (id: string, field: keyof LigneDevis, value: string | number) => {
    setFormData(prev => ({
      ...prev,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-secondary-50 rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="block text-sm font-medium text-secondary-700 mb-1">Rechercher un client</label>
            <input
              type="text"
              name="clientNom"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-secondary-700 mb-1">Validité du devis</label>
          <input
            type="date"
            name="dateValidite"
        ...
        <div>
          <label className="block text-sm font-medium text-secondary-700 mb-1">Conditions de paiement</label>
          <select
      ...
      {/* Notes */}
      <div>
        <label className="block text-sm font-medium text-secondary-700 mb-1">Notes / Conditions particulières</label>
        <textarea
          name="notes"
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-secondary-100">
        ...
        >
          ...
          {isLoading ? 'Enregistrement...' : 'Créer le devis'}
        </button>
      </div>
    </form>
  );
